        ...
        <tbody>
          @foreach ($clients as $item)
            <tr>
              ...
              <td>
                {{ $item->sales->sortByDesc('created_at')->first() ? date('d-m-y', strtotime($item->sales->sortByDesc('created_at')->first()->created_at)) : 'N/A' }}
              </td>
              <td>
                <div class="d-flex align-items-center justify-content-end">
                  <a href="{{ route('clientShow', $item->id) }}" type="button"
                  ...
                  <a href="#" class="btn btn-sm btn-icon btn-icon-start btn-outline-info ms-1"
                    data-bs-toggle="modal" data-bs-target="#clientEdit{{ $item->id }}" type="button">
                  ...
                  <button class="btn btn-sm btn-icon btn-icon-start btn-outline-danger ms-1" type="button"
                    data-bs-toggle="modal" data-bs-target="#clientDelete{{ $item->id }}">
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 20 20"
                      fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
