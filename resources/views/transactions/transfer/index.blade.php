...
<div class="page-title-container">
    <div class="row">
        ...
        <div class="col-12 col-sm-6 d-flex align-items-start justify-content-end">
                    <!-- Tour Button Start -->
            <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#transfersRegister">Register Transfers</button>
            <!-- Tour Button End -->
        </div>
    </div>
</div>
            ...
            <tbody>
             @forelse ($transfers as $transfer)
                ...
                <tr>
                    ...
                    <td><a href="{{ route('method-view', $transfer->sender_payment_method) }}">{{ $transfer->sender_payment_method->name }}</a></td>
                    <td><a href="{{ route('method-view', $transfer->receiver_payment_method) }}">{{ $transfer->receiver_payment_method->name }}</a></td>
                    <td>{{ $transfer->reference }}</td>
                    <td>₦{{ $transfer->sender_method }}</td>
                    <td>₦{{ $transfer->receiver_method }}</td>
                    <td>
                    @role('Admin Manager')
                    <button class="btn btn-sm btn-icon btn-icon-start btn-outline-danger ms-1" type="button" data-bs-toggle="modal" data-bs-target="#transferDelete{{ $transfer->id }}">
